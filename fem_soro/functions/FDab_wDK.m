function  qdd = FDab_wDK( model, q, qd, tau, f_ext )
...
S = repmat({zeros(3,1)},1,100);
Xup = repmat({zeros(3,3)},1,100);
v = repmat({zeros(3,1)},1,100);
c = repmat({zeros(3,1)},1,100);
IA = repmat({zeros(3,3)},1,100);
pA = repmat({zeros(3,1)},1,100);
a = repmat({zeros(3,1)},1,100);
U = repmat({zeros(3,1)},1,100);
d = repmat({0},1,100);
u = repmat({0},1,100);
...
for i = 1:model.NB
  [ XJ, S{i} ] = jcalc_wDK( q(i) );%model.jtype{i}, q(i) );
  vJ = S{i}*qd(i);
