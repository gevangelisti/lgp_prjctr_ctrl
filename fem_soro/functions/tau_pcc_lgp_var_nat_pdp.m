function [tau] = tau_pcc_lgp_var_nat_pdp(t,chi_,xi_,a_des,omega_d,...
...
tau_ = M_est*ddq_des(t) + C_est*dq_des(t) ... 
    + (eye(N)-heaviside_man(e'*nabla_chi_g_est)*(e*e')./(1e-3 + norm(e)^2) )*nabla_chi_g_est ...
    - ge - K_vfb(K1,K2,K3,Sigma_lgp_temp,e) ...
    + (eye(N)-heaviside_man(de_'*d)*(de_*de_')./(1e-3 + norm(de_)^2) )*d ...
    - de - Kd*de_ - K_vfb(K1,K2,K3,Sigma_lgp_temp,de); 

ddq_ = M_est \ (tau_ - C_est * xi_ - nabla_chi_g_est - diag(pb.d)*xi_);
